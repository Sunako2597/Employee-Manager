...
{
  // $marksql = "SELECT  mark FROM `project` WHERE project.eid = '$id' AND pid = '$pid'";
  // $resultOM = mysqli_query($conn, $marksql);
  // $resOM = mysqli_fetch_assoc($resultOM);
  // $oldMark = $resOM['mark'];
  // echo "om: $oldMark";
  ...
  $oldMark = mysqli_real_escape_string($conn, $_POST['oldMark']);
  $newMark = mysqli_real_escape_string($conn, $_POST['newMark']);
  $points = mysqli_real_escape_string($conn, $_POST['points']);
  $base = mysqli_real_escape_string($conn, $_POST['base']);
  $bonus = mysqli_real_escape_string($conn, $_POST['bonus']);
  $total = mysqli_real_escape_string($conn, $_POST['total']);

  echo"$newMark $oldMark";
  $markDifference = $newMark - $oldMark;
  $upPoint = $points+$markDifference;
  
  if($markDifference>0){
  $upBonus = $bonus +  $markDifference;
 }
  else{
    $upBonus = $bonus;
  }
 ...
 $result = mysqli_query($conn, "UPDATE `project` SET `mark`='$newMark' WHERE eid='$eid' and pid = '$pid'");
...
<html>
<head>
  <title>Project Mark | Emplo Guru</title>
  <!-- Icons font CSS-->
            ...
            <div class="card card-1">
                ...
                <div class="card-body">
                    ...
                    <form id = "registration" action="mark.php" method="POST">
                        ...
                        <div class="input-group">
                          <p>Assign Mark</p>
                            <input class="input--style-1" type="text"  name="newMark" value="<?php echo $oldMark;?>">
                        </div>

                        <input type="hidden" name="oldMark" id="textField" value="<?php echo $oldMark;?>" required="required">
